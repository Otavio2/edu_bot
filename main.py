...
def salvar_memoria():
    with open(MEMORY_FILE, "w", encoding="utf-8") as f:
        json.dump(memory_cache[-2000:], f, ensure_ascii=False, indent=2)
...
def carregar_memoria_do_canal():
    ...
    limit = 100
    todos_itens = []
    print("Carregando memória do canal... pode demorar na 1ª vez")
    ...
    while True:
        ...
        try:
            ...
            if len(mensagens) < limit:
                break
            offset += limit
            time.sleep(0.1) # pra não tomar flood
...
def carregar_memoria():
    ...
    if os.path.exists(MEMORY_FILE):
        try:
            ...
            memory_cache = []
    carregar_memoria_do_canal() # sempre atualiza com o canal

def salvar_no_canal(user_id, user_msg, bot_reply):
    palavras_chave = ["eu gosto", "meu nome", "eu sou", "trabalho", "moro", "tenho", "faço"]
    if any(p in user_msg.lower() for p in palavras_chave):
...
def gerar_resumo_usuario(user_id):
    ...
    if not infos: return ""
    texto = "\n".join([f"Usuário: {i['user']}" for i in infos[-20:]])
    prompt = f"Resuma em 5 linhas máximo o que você sabe sobre esta pessoa. Seja direto: {texto}"
    ...
    if "text" in message:
        ...
        if chat_type == "private":
            ...
            if reply_to:
                ...
                respondendo_bot = reply_from.get("username", "").lower() == u_clean or reply_from.get("first_name", "").lower() == BOT_FIRST_NAME.lower()
