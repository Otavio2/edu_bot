...
user_timezones = {}
group_ids = set()
group_languages = {}  # idioma do grupo para postagens automáticas
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    f"Você é {BOT_NAME}, bot criado por {CREATOR_NAME}. {creator_info} "
                    "Responda sempre no idioma enviado pelo usuário, seja curto, natural, direto, com emojis quando fizer sentido. "
                    f"{time_info}"
                )
            }
        ] + history
...
# --- Quiz com enquetes nativas e exclusão automática ---
...
def webhook():
    ...
    if chat_type in ["group", "supergroup"]:
        ...
        # Detecta idioma do grupo pelo first message (simplificado)
    ...
    if "text" in message:
        ...
        if chat_type == "private":
            ...
            if BOT_NAME.lower() in user_msg.lower():
                should_reply = True
            elif BOT_USERNAME and BOT_USERNAME.lower() in user_msg.lower():
                should_reply = True
            elif message.get("reply_to_message", {}).get("from", {}).get("username", "").lower() == BOT_USERNAME.lower().replace("@", ""):
                should_reply = True
        ...
        if should_reply:
            try:
                if user_msg.lower().startswith("/piada"):
                    post = get_joke_api()
                    reply = groq_chat(message["from"]["id"], f"Traduza e adapte para o idioma do usuário: {post}")
                    send_telegram_message(chat_id, f"PIADA\n🤣 {reply}", reply_to_message_id=message.get("message_id"))
                    # Apaga comando depois de 5 segundos
                    ...
                    send_translated_quiz(chat_id, message["from"]["id"], user_lang)
                    # Apaga comando depois de 5 segundos
                    scheduler.add_job(lambda: requests.post(
                        f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/deleteMessage",
                        json={"chat_id": chat_id, "message_id": message.get("message_id")},
                    ...
                    reply = groq_chat(message["from"]["id"], user_msg)
...
@app.route("/")
def index():
    return f"{BOT_NAME} rodando com estilo criativo! Criado por {CREATOR_NAME} 🎭"
